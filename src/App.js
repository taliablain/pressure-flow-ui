...
function App() {
 ...
 useEffect(() => {
   const tempTimer = setInterval(() => {
     ...
     const targetPressure = basePressure + (pressureWave * pressureRange) + (pressureMicroWave * pressureRange);
     
     // Ensure pressure stays within bounds (0.5 to 1.5)
     const clampedTargetPressure = Math.max(0.5, Math.min(1.5, targetPressure));
     ...
     const paraffinSecondaryWave = Math.sin(cycleProgress * 12 * Math.PI) * 0.3; // Faster smaller fluctuations
     const paraffinTertiaryWave = Math.cos(cycleProgress * 18 * Math.PI) * 0.15; // Even faster micro-fluctuations
     
     // Combine waves for complex, realistic fluctuation pattern
     const paraffinCombinedWave = paraffinPrimaryWave + paraffinSecondaryWave + paraffinTertiaryWave;
     ...
     let paraffinTargetTemp = paraffinBaseTemp + (paraffinCombinedWave * paraffinTempRange);
     ...
     const clampedParaffinTargetTemp = Math.max(300, Math.min(350, paraffinTargetTemp));
     ...
     const keroseneSecondaryWave = Math.sin(cycleProgress * 11 * Math.PI) * 0.3; // Faster smaller fluctuations
     const keroseneTertiaryWave = Math.cos(cycleProgress * 17 * Math.PI) * 0.15; // Even faster micro-fluctuations
     
     // Combine waves for complex, realistic fluctuation pattern
     const keroseneCombinedWave = kerosenePrimaryWave + keroseneSecondaryWave + keroseneTertiaryWave;
     ...
     const keroseneTargetTemp = keroseneBaseTemp + (keroseneCombinedWave * keroseneTempRange);
     
     // Ensure kerosene temperature stays within bounds
     const clampedKeroseneTargetTemp = Math.max(250, Math.min(300, keroseneTargetTemp));
     
     // KEROSENE PRESSURE CALCULATION
     // Similar to cracker but different frequency
     const kerosenePressureWave = Math.sin(cycleProgress * 1.5 * Math.PI) * 0.7; // Slow sine wave
     const kerosenePressureMicroWave = Math.sin(cycleProgress * 7 * Math.PI) * 0.1; // Small variations
     
     // Target pressure between 0.5 and 1.5 bar
     const keroseneBasePressure = 1.0;
     const kerosenePressureRange = 0.4; // ±0.4 from base
     const keroseneTargetPressure = keroseneBasePressure + (kerosenePressureWave * kerosenePressureRange) + (kerosenePressureMicroWave * kerosenePressureRange);
     
     // Ensure kerosene pressure stays within bounds (0.5 to 1.5)
     const clampedKeroseneTargetPressure = Math.max(0.5, Math.min(1.5, keroseneTargetPressure));
     ...
     // Combine waves for complex, realistic fluctuation pattern
     const dieselCombinedWave = dieselPrimaryWave + dieselSecondaryWave + dieselTertiaryWave;
     ...
     const dieselTargetTemp = dieselBaseTemp + (dieselCombinedWave * dieselTempRange);
     
     // Ensure diesel temperature stays within bounds
     const clampedDieselTargetTemp = Math.max(150, Math.min(250, dieselTargetTemp));
     ...
     const dieselPressureMicroWave = Math.sin(cycleProgress * 6.5 * Math.PI) * 0.1; // Small variations
     
     // Target pressure between 0.5 and 1.5 bar
     const dieselBasePressure = 1.0;
     const dieselPressureRange = 0.35; // ±0.35 from base
     const dieselTargetPressure = dieselBasePressure + (dieselPressureWave * dieselPressureRange) + (dieselPressureMicroWave * dieselPressureRange);
     
     // Ensure diesel pressure stays within bounds (0.5 to 1.5)
     const clampedDieselTargetPressure = Math.max(0.5, Math.min(1.5, dieselTargetPressure));
     ...
     // Combine waves for complex, realistic fluctuation pattern
     const petrolCombinedWave = petrolPrimaryWave + petrolSecondaryWave + petrolTertiaryWave;
     ...
     const petrolTargetTemp = petrolBaseTemp + (petrolCombinedWave * petrolTempRange);
     
     // Ensure petrol temperature stays within bounds
     const clampedPetrolTargetTemp = Math.max(30, Math.min(40, petrolTargetTemp));
     ...
     const petrolPressureMicroWave = Math.sin(cycleProgress * 5 * Math.PI) * 0.05; // Very small variations
     ...
     const petrolTargetPressure = petrolBasePressure + (petrolPressureWave * petrolPressureRange) + (petrolPressureMicroWave * petrolPressureRange);
     
     // Ensure petrol pressure stays within tight bounds around 1.0
     const clampedPetrolTargetPressure = Math.max(0.9, Math.min(1.1, petrolTargetPressure));
     ...
     setTimeout(() => {
       ...
       setParaffinPressure(prevPressure => {
         ...
         if (Math.random() < 0.02) {
           ...
           const clampedPressure = Math.max(0.85, Math.min(1.15, finalPressure));
     ...
     <div className="distillation-container">
             ...
             <div className="measurement-boxes">
               <div className="measurement-box temperature">
                 <span className="measurement-value">
                   {product === 'PARAFFIN' ? paraffinTemp.toFixed(1) : 
                    product === 'KEROSENE' ? keroseneTemp.toFixed(1) :
                    product === 'DIESEL' ? dieselTemp.toFixed(1) :
                    product === 'PETROL' ? petrolTemp.toFixed(1) : '25.0'}
                 </span>
                 <span className="measurement-unit">°C</span>
               </div>
               <div className="measurement-box pressure">
                 <span className="measurement-value">
                   {product === 'PARAFFIN' ? paraffinPressure.toFixed(1) : 
                    product === 'KEROSENE' ? kerosenePressure.toFixed(1) :
                    product === 'DIESEL' ? dieselPressure.toFixed(1) :
                    product === 'PETROL' ? petrolPressure.toFixed(1) : '1.2'}
                 </span>
       ...
       <div className="product-column">
         ...
         <div className="product-circle">
           <div className="measurement-boxes">
             <div className="measurement-box temperature">
               <span className="measurement-value">{crackerTemp.toFixed(1)}</span>
               <span className="measurement-unit">°C</span>
             </div>
             <div className="measurement-box pressure">
